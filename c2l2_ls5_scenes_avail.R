...
## write list of scenes with cloud cover <60%
write.table(scenes.avail[which(scenes.avail$Land.Cloud.Cover <= 60),2], quote=FALSE, ## 574 scenes
            '/Users/Avril/Desktop/C2L2_intermediate_cloud_scenes.txt', sep='\t', row.names=FALSE, col.names=FALSE)
...
## scenes with <= 20% cloud cover
temp <- scenes.avail[which(scenes.avail$Land.Cloud.Cover <= 20),]
cloud.col <- 1-(temp$Scene.Cloud.Cover/100)
cloud.col[which(cloud.col==0)] <- 0.01
pdf('/Users/Avril/Desktop/C2L2_landsat5_data_across_years_20percentcloudcover.pdf', width=20, height=7)
par(mar=c(5.1, 5.1, 6.1, 2.1))
