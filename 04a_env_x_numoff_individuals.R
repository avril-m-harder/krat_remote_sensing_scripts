...
prism.dat[which(prism.dat$month %in% c(7:12)), 'half.year.lag'] <- prism.dat[which(prism.dat$month %in% c(7:12)), 'year'] + 1
prism.dat[which(prism.dat$month %in% c(1:6)), 'half.year.lag'] <- prism.dat[which(prism.dat$month %in% c(1:6)), 'year']
...
#   save <- as.numeric(c(unlist(fem.ann[r,c(2:5)]), '1', NA, g, b, w, t, pp, pt, g.sd, b.sd, w.sd, t.sd))
#   OUT <- rbind(OUT, save)
...
#     w.sd <- sd(temp$z.w)
#     t.sd <- sd(temp$temp.k)
#     save <- as.numeric(c(unlist(fem.ann[r,c(2:5)]), '2', s, g, b, w, t, pp, pt, g.sd, b.sd, w.sd, t.sd))
#     OUT <- rbind(OUT, save)
#   }
# }
...
env.fem.ann <- read.csv('/Users/Avril/Documents/krat_remote_sensing/intermediate_data/indiv_numoff_x_env_bycells.csv')
...
##### Construct linear models and conduct LRTs (number of offspring) #####
##### >> Annual averages / totals #####
sub <- env.fem.ann[env.fem.ann$type == 1,]        ## subset to annual summary stats

mod1 <- lm(sub$num.off ~ sub$mean.g + sub$mean.b + sub$mean.w + sub$mean.tempk)
summary(mod1) ## remove mean.w

mod2 <- lm(sub$num.off ~ sub$mean.g + sub$mean.b + sub$mean.tempk)
lrtest(mod1, mod2) ## not a significant improvement from mod1 --> mod2
summary(mod2) ## remove mean.g

mod3 <- lm(sub$num.off ~ sub$mean.b + sub$mean.tempk)
lrtest(mod2, mod3) ## not a significant improvement from mod2 --> mod3
summary(mod3) ## all variables significant, but R2 is suuuuuper low
...
mod2 <- lm(sub$num.off ~ sub$mean.b + sub$mean.w + sub$mean.tempk)
...
mod4 <- lm(sub$num.off ~ sub$mean.b)
summary(mod4) ## all variables significant, but R2 is super low
lrtest(mod3, mod4) ## not a significant improvement from mod3 --> mod4

## mixed effects
# res <- glmer(num.off ~ mean.g + mean.b + mean.w + mean.tempk + (1|year), data = sub, family = 'poisson')
# summary(res)
# rsquared(res)
...
mod2 <- lm(sub$num.off ~ sub$mean.b + sub$mean.w + sub$mean.tempk)
summary(mod2) ## remove mean.b
lrtest(mod1, mod2) ## not a significant improvement from mod1 --> mod2

mod3 <- lm(sub$num.off ~ sub$mean.w + sub$mean.tempk)
lrtest(mod2, mod3) ## not a significant improvement from mod2 --> mod3
summary(mod3) ## significant, but very low R2

shapiro.test(residuals(mod3))
plot(mod3)

##### !!! ADD ASSUMPTION CHECKS #####
pdf('/Users/Avril/Desktop/numoff_ttests.pdf', width = 5, height = 5)
k <- 1
while(k == 1){
##### T-tests for +/- offspring produced #####
##### >> Annual averages / totals #####
sub <- env.fem.ann[env.fem.ann$type == 1,]        ## subset to annual summary stats
sub[sub$num.off == 0, 'off.or.no'] <- 1
sub[sub$num.off > 0, 'off.or.no'] <- 2
...
plot(jitter(sub$off.or.no, factor = 0.8), sub$mean.g, xlim = c(0.6, 2.4), pch = 19,
     col = 'transparent', xaxt = 'n', xlab = 'Number of offspring',
