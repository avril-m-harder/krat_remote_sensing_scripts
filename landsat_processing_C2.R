...
# setwd('/Volumes/avril_data/krat_remote_sensing/C2L2_raw_landsat45tm_scene_downloads/') #### !!! move extra files out of this dir first !!!
dirs <- list.files()
...
for(i in dirs){
  setwd(paste0('/Volumes/avril_data/krat_remote_sensing/C2L2_raw_landsat45tm_scene_downloads/',i,'/'))
  ## read in data
  ...
  writeRaster(ls5.stack, filename=paste0('/Volumes/avril_data/krat_remote_sensing/C2L2_croppped_landsat45tm_scenes/',i,'_CROPPED.grd'), progress='text', overwrite=TRUE)
...
#### 3A. Initial test run of this loop = 5 minutes ####
...
##### *3A. Need to manually review both PDFs (cloud_qa_fails.pdf and test.pdf) to make sure that QA didn't misclassify images #####

# #### 3B. Analyzing cloud-free scenes only = 5 minutes ####
# ## set file names for run
...
# hi.y <- 3499750
# 
# ## get list of cloud-free scenes
# cf.scenes <- read.table('/Users/Avril/Documents/krat_remote_sensing/landsat_5_data_overviews/cloud_free_prodID.txt')
# 
# # setwd('/Volumes/avril_data/krat_remote_sensing/cropped_landsat45tm_scenes/')
# setwd('/Users/Avril/Documents/krat_remote_sensing/cropped_landsat45tm_scenes/')
# files <- paste0(cf.scenes$V1,'_CROPPED.grd')
# 
# pdf(file='/Users/Avril/Desktop/test.pdf', width=8, height=8)
# par(mar=c(3.1,2.1,2.1,1.1), mgp=c(1.5,.75,0))
# 
...
#   print(i/length(files))
# }
# print(paste0('Loop finish: ',Sys.time()))
# dev.off()
# 
# ## plot scenes with problematic cloud cover, according to QA band
# pdf('/Users/Avril/Desktop/cloud_qa_fails.pdf', width=8, height=8)
